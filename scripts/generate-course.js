...
if (!courseId) { console.error('Usage: node generate-course.js <course>'); process.exit(1); }
...
if (!course) { console.error('Unknown course:', courseId); process.exit(1); }

const chs = course.chapters;
const totalXP = chs.length * course.xp;

function genQ(idx, chName) {
  const prefix = course.n + '-ch' + (idx + 1);
...
for (let i = 0; i < chs.length; i++) {
  ...
  out += `    {
      ...
      sections: [
        ...
        { type: "code", language: "${course.n === 'javascript' ? 'javascript' : course.n === 'python' ? 'python' : course.n === 'css' ? 'css' : 'html'}", caption: "A practical example demonstrating ${clean}", content: \`<!-- ${clean} Example -->\\n<div class="container">\\n  <p>This demonstrates key concepts of ${clean}.</p>\\n</div>\` },
...
fs.writeFileSync(path.join(__dirname, '..', 'src', 'lib', 'courses', course.n, 'index.ts'), out);
console.log('Generated ' + course.n + ' course: ' + chs.length + ' chapters');
